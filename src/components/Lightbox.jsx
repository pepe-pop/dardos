...
/** Pełnoekranowy podgląd zdjęcia LUB filmu z podpisem i nawigacją. */
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-black/95" onClick={onClose}>
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <span className="text-sm font-semibold text-muted">
          {isVideo ? '🎬 ' : ''}{photo.author} {photo.year && `• ${photo.year}`}
        </span>
      ...
      <div className="px-5 pb-8 pt-2 text-center" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mt-1 text-xs text-muted">
          {isVideo ? 'dotknij ▶ aby odtworzyć' : 'dotknij, aby zamknąć'}
        </p>
      </div>
    </div>
  )
