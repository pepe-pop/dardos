...
9. Build → **Firestore Database** → Utwórz (tryb testowy, potem reguły) → skopiuj dane projektu.
10. Build → **Storage** → Rozpocznij (domyślny bucket).
11. Build → Ustawienia projektu → **Twoje aplikacje** → Aplikacja web `</>` → skopiuj `apiKey` itd. → wklej do `src/firebase-config.js`.
12. Ustaw `FEATURES.storageMode = 'firebase'` w `src/config.js`.
13. Wgraj reguły: `firebase/firestore.rules` i `firebase/storage.rules` (konsola → Firestore → Reguły; Storage → Reguły) — pamiętaj o zmianie sekretu admina w obu plikach.
14. `npm run build` → commit → push → Actions wdroży nową wersję. Przetestuj na telefonie: dodanie zdjęcia, zdania, gra.
...
└── src/
    ...
    ├── firebase-config.js              # ★ dane projektu Firebase (wypełnij w produkcji)
